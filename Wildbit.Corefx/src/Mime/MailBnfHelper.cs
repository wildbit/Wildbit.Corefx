...
namespace Postmark.Corefx.Mime
{
    internal static class MailBnfHelper
    {
        ...
        internal static bool SkipCFWS(string data, ref int offset)
        {
            ...
            {
                if (data[offset] > 127)
                    throw new FormatException(string.Format(Strings.MailHeaderFieldInvalidCharacter, data[offset]));
        ...
        internal static void ValidateHeaderName(string data)
        {
            ...
            for (; offset < data.Length; offset++)
            {
                if (data[offset] > Ftext.Length || !Ftext[data[offset]])
                    throw new FormatException(Strings.InvalidHeaderName);
            }
            if (offset == 0)
                throw new FormatException(Strings.InvalidHeaderName);
        }
        ...
        private static bool CheckForUnicode(char ch, bool allowUnicode)
        {
            ...
            if (!allowUnicode)
            {
                throw new FormatException(string.Format(Strings.MailHeaderFieldInvalidCharacter, ch));
            }
            return true;
        }
