...
namespace Wildbit.Corefx.Mail
{
    ...
    internal static class AuthCommand
    {
        ...
        private static LineInfo CheckResponse(LineInfo[] lines)
        {
            ...
            {
                throw new SmtpException(Strings.SmtpAuthResponseInvalid);
            }
    ...
    internal static class DataCommand
    {
                ...
                default:
                    {
                        ...
                        {
                            throw new SmtpException(Strings.net_webstatus_ServerProtocolViolation, serverResponse);
                        }
        ...
        private static void PrepareCommand(SmtpConnection conn)
        {
            ...
            {
                throw new InvalidOperationException(Strings.SmtpDataStreamOpen);
            }
    ...
    internal static class EHelloCommand
    {
        ...
        private static string[] CheckResponse(LineInfo[] lines)
        {
            if (lines == null || lines.Length == 0)
            {
                throw new SmtpException(Strings.SmtpEhloResponseInvalid);
            }
            if (lines[0].StatusCode != SmtpStatusCode.Ok)
            {
                if ((int)lines[0].StatusCode < 400)
                {
                    throw new SmtpException(Strings.net_webstatus_ServerProtocolViolation, lines[0].Line);
                }
                ...
                default:
                    {
                        if ((int)statusCode < 400)
                        {
                            throw new SmtpException(Strings.net_webstatus_ServerProtocolViolation, response);
                        }
