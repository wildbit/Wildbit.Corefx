    ...
    public class SmtpClient : IDisposable
    {
        ...
        public string Host
        {
            ...
            set
            {
                if (InCall)
                {
                    throw new InvalidOperationException(Strings.SmtpInvalidOperationDuringSend);
                }
                ...
                {
                    throw new ArgumentException(Strings.net_emptystringset, nameof(value));
                }
        ...
        internal MailWriter GetFileMailWriter(string pickupDirectory)
        {
            ...
            if (!Path.IsPathRooted(pickupDirectory))
                throw new SmtpException(Strings.SmtpNeedAbsolutePickupDirectory);
            string filename;
        ...
        private void CheckHostAndPort()
        {
            if (_host == null || _host.Length == 0)
            {
                throw new InvalidOperationException(Strings.UnspecifiedHost);
            }
        ...
        private void Complete(Exception exception, IAsyncResult result)
        {
            ...
            try
            {
                ...
                else if (exception != null && (!(exception is SmtpFailedRecipientException) || ((SmtpFailedRecipientException)exception).fatal))
                {
                    ...
                    {
                        exception = new SmtpException(Strings.SmtpSendMailFailure, exception);
                    }
                }
                else
                {
